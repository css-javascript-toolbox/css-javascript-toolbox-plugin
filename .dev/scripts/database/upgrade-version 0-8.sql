...
INSERT INTO 60wp35_options (option_name, option_value) select option_name, option_value from `08_options` where option_name = 'cjtoolbox_data';

/* Blocks Order */
DELETE FROM 60wp35_usermeta WHERE meta_key = 'meta-box-order_settings_page_cjtoolbox';
INSERT INTO 60wp35_usermeta (user_id, meta_key, meta_value) select 1, meta_key, meta_value from `08_usermeta` where user_id = 1 and meta_key = 'meta-box-order_settings_page_cjtoolbox' ;
...
INSERT INTO 60wp35_usermeta (user_id, meta_key, meta_value) select 1, meta_key, meta_value from `08_usermeta` where user_id = 1 and meta_key = 'closedpostboxes_settings_page_cjtoolbox' ;
...
INSERT INTO 60wp35_options (option_name, option_value) SELECT option_name, option_value FROM 08_options WHERE option_name = 'cjtoolbox_tools_backups';

/* Copy Template table and data. */
DROP TABLE IF EXISTS 60wp35_cjtoolbox_cjdata;
CREATE TABLE 60wp35_cjtoolbox_cjdata LIKE `08_cjtoolbox_cjdata`;
INSERT INTO 60wp35_cjtoolbox_cjdata SELECT * FROM `08_cjtoolbox_cjdata`;
...
/* Access Points Cache*/
DELETE FROM 60wp35_options WHERE option_name = 'settings.CJTAccessPointsDirectorySpider.cache';
